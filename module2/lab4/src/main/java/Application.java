...
public class Application extends JFrame {
    ...
    private static IBackend backend = null;
    ...
    private Application() throws RemoteException {
        ...
        comboPolygon.addActionListener(e -> {
            try {
                currentPolygon = backend.polygonFindByName((String) comboPolygon.getSelectedItem());
        ...
        comboVertex.addActionListener(e -> {
            try {
                currentVertex = backend.vertexFindByName((String) comboVertex.getSelectedItem());
            } catch (RemoteException remoteException) {
                remoteException.printStackTrace();
            }
            polygonMode = false;
    ...
    private static void save() throws RemoteException {
        if (editMode) {
            if (polygonMode) {
                ...
                if (!backend.polygonUpdate(currentPolygon)) {
                ...
                PolygonDTO polygon = backend.polygonFindByName(textVertexPolygonName.getText());
                if (polygon == null) {
                ...
                if (!backend.vertexUpdate(currentVertex)) {
                    JOptionPane.showMessageDialog(null, "Error: update failed!");
                }
            }
        } else {
            if (polygonMode) {
                ...
                if (!backend.polygonInsert(polygon)) {
                ...
                if (!backend.vertexInsert(vertex)) {
                    JOptionPane.showMessageDialog(null, "Error: insertion failed!");
                    return;
                }
    ...
    private static void delete() throws RemoteException {
        if (editMode) {
            if (polygonMode) {
                List<VertexDTO> list = backend.vertexFindByPolygonId(currentPolygon.getId());
                assert list != null;
                for (VertexDTO vertex : list) {
                    comboVertex.removeItem(vertex.getName());
                    backend.vertexDelete(vertex);
                }
                backend.polygonDelete(currentPolygon);
                comboPolygon.removeItem(currentPolygon.getName());

            } else {
                backend.vertexDelete(currentVertex);
    ...
    private void fillComboBoxes() throws RemoteException {
        ...
        List<PolygonDTO> polygons = backend.polygonAll();
        List<VertexDTO> vertexes = backend.vertexAll();
    ...
    private static void fillVertexFields() throws RemoteException {
        if (currentVertex == null) {
            return;
        }
        PolygonDTO polygon = backend.polygonFindById(currentVertex.getPolygonName());
        assert polygon != null;
    ...
    public static void main(String[] args) throws IOException, NotBoundException {
        String url = "//localhost:8085/graphics";
        backend = (IBackend) Naming.lookup(url);
